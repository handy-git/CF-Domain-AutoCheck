...
<details>
    ...
    ☑️2026-08-26 <br>
    1.彻底移除Telegram通知，改为Resend HTML邮件通知 <br>
    2.邮件支持美观的HTML模板（状态徽章、续期按钮、统计卡片） <br>
    <br>
    ☑️2025-09-12 <br>
...
* 主要功能：日期监控、价格记录、注册商记录、自定义标签、自定义续费链接、HTML邮件提前通知。
...
5. 设定环境变量，cloudflare环境变量名如下：
    ...
    | MAIL_API_KEY      | 去[Resend](https://resend.com/)注册获取API Key                                |  ✅️   | 发送到期提醒邮件，必填                   |
    | MAIL_TO           | 收件人邮箱，用于接收到期提醒邮件                                               |  ✅️   | 必填                                     |
    | MAIL_FROM         | 发件人邮箱，可选                                                               |  ❌️   | 不填默认用 onboarding@resend.dev，建议验证域名后换成自己的 |
...
> 邮件通知由 [Resend](https://resend.com/) 提供发送服务，免费额度 100 封/天、3000 封/月。只需配置 `MAIL_API_KEY` 和 `MAIL_TO` 即可使用；`MAIL_FROM` 可选，不配置时默认用 `onboarding@resend.dev` 测试发送（仅可发送到你注册 Resend 时填写的邮箱），建议在 Resend 验证自己的发件域名后改为 `alert@你的域名`。

6. 按照上述变量名添加完邮件变量后，点击`设置`——点击`触发事件`——点击`添加`——选择`cron触发器`——选择`一周中的某一天`——自定义时间
